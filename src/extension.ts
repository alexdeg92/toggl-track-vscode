...
# Use python3 for proper JSON escaping and API call (fire and forget)
python3 -c "
...
# Strip Monday task link from message
lines = [l for l in commit_msg.split(chr(10)) if not l.startswith('Monday task:')]
clean_msg = chr(10).join(lines).strip()

body = f'Commit {commit_hash} by {commit_author}:\n\n{clean_msg}'
query = f'mutation {{ create_update(item_id: \$TASK_ID, body: {json.dumps(body)}) {{ id }} }}'
