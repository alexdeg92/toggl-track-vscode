...
# Monday.com API token (embedded from VS Code settings at hook install time)
MONDAY_TOKEN="\${MONDAY_TOKEN:-${token}}"
...
# Extract Monday task ID from branch name
TASK_ID=\$(echo "\$BRANCH" | grep -oE '[0-9]{8,}' | head -1)

# Fallback: check .vscode/monday-tasks.json
if [ -z "\$TASK_ID" ] && [ -f ".vscode/monday-tasks.json" ]; then
  TASK_ID=\$(python3 -c "import json; d=json.load(open('.vscode/monday-tasks.json')); print(d.get('\$BRANCH',{}).get('taskId',''))" 2>/dev/null)
fi
...
# Get commit message (subject + body), strip the Monday task link line
COMMIT_MSG=\$(git log -1 --pretty=format:"%s%n%n%b" | sed '/^Monday task:/d' | sed '/^$/N;/^\\n$/d')
COMMIT_HASH=\$(git log -1 --pretty=format:"%h")
COMMIT_AUTHOR=\$(git log -1 --pretty=format:"%an")

# Build the update text
UPDATE_TEXT="🔨 Commit \$COMMIT_HASH by \$COMMIT_AUTHOR:\\n\\n\$COMMIT_MSG"

# Escape for JSON
UPDATE_JSON=\$(echo "\$UPDATE_TEXT" | python3 -c "import sys,json; print(json.dumps(sys.stdin.read().strip()))")

# Post update to Monday.com (fire and forget, don't block the commit)
curl -s -X POST "https://api.monday.com/v2" \\
  -H "Authorization: \$MONDAY_TOKEN" \\
  -H "Content-Type: application/json" \\
  -d "{\\"query\\": \\"mutation { create_update(item_id: \$TASK_ID, body: \$UPDATE_JSON) { id } }\\"}" \\
  > /dev/null 2>&1 &
